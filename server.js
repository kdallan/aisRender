...
class CallSession {
  constructor(webSocket, appServices) {
    ...
    // For accumulating audio packets
    this.audioAccumulator = [];
    this.audioAccumulatorSize = 0; // Total accumulated size in bytes
    // Commit if accumulated audio exceeds this threshold (16 KB in this example)
    this.bufferSizeThreshold = 16 * 1024;
    ...
    this._flushAudioBuffer = this._flushAudioBuffer.bind(this);
    this._accumulateAudio = this._accumulateAudio.bind(this);
            ...
            if (data.media.track === "inbound" || data.media.track === "outbound") {
              ...
              try {
                ...
                if (hasAudio) {
                  ...
                  this._accumulateAudio(rawAudio);
                } else {
                  this.silencePackets++;
                }
              } catch (error) {
  ...
  _accumulateAudio(buffer) {
    this.audioAccumulator.push(buffer);
    this.audioAccumulatorSize += buffer.length;
    
    // If the accumulated buffer exceeds the threshold, flush immediately.
    ...
    // Otherwise, if no timer is active, set one.
    if (!this.audioAccumulatorTimer) {
  ...
  _flushAudioBuffer() {
    ...
    // Combine all accumulated buffers into one.
    const combinedBuffer = Buffer.concat(this.audioAccumulator);
    
    if (this.sttService && this.sttService.connected) {
      logger.debug(`Flushing ${this.audioAccumulator.length} buffers combined, total size: ${combinedBuffer.length} bytes`);
