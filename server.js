...
const logger = {
  ...
  trace: (message, data) => {
    if (process.env.TRACE) console.log(`[TRACE] ${message}`, data || "");
  },
};
    ...
    return {
      ...
      twilio: {
        ...
        studioFlowId:
          ...
          "FWe2a7c39cffcbe604f2f158b68aae3b19",
      },
      deepgram: {
        ...
        sttConfig: {
          ...
          utterance_end_ms:
            parseInt(process.env.DEEPGRAM_UTTERANCE_END_MS) || 800,
        },
        throttleInterval:
...
class TextUtils {
  constructor(config) {
    this.punctuationChars = config.chars;
  ...
  containsAnyPunctuation(text) {
    ...
    return Array.from(text).some((char) =>
      this.punctuationChars.includes(char)
    );
  }
      ...
      this.deepgram.addListener(LiveTranscriptionEvents.Transcript, (data) => {
        try {
          if (!data.is_final) {
            ...
            if (transcript) this.onTranscript && this.onTranscript(transcript, false);
            return;
          }
          const transcript = data.channel?.alternatives?.[0]?.transcript;
...
const parseIncomingMessage = (message) => {
  ...
  try {
    ...
    logger.debug("Failed to parse incoming message", error);
  }
...
class CallSession {
  ...
  async _handleMessage(message) {
    ...
    if (!data) return;
    if (!this.hasSeenMedia) logger.debug(`Complete message: ${messageText}`);
    switch (data.event) {
      case "connected":
