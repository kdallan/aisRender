...
class CallSession {
  constructor(webSocket, appServices) {
    ...
    // Flush the audio queue every 100ms (configurable via env)
    this.batchFlushInterval = parseInt(process.env.AUDIO_BATCH_FLUSH_INTERVAL) || 100;
  ...
  _flushAudioBufferQueue() {
    if (this.audioBufferQueue.length === 0 || !this.active) return;
